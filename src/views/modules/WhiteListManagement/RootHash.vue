...
export default {
  ...
  methods: {
    ...
    async createRootHash() {
      ...
      this.leafNodes = this.whitelistAddresses.map((address) =>
        keccak256(address)
      );
    ...
    async setMerkle() {
      try {
        ...
        // 调用智能合约设置Merkle的根哈希
        let transaction = await contract.TmetoNFTContract.setSaleMerkleRoot(
          this.rootHash
        );
        ...
        if (data) {
          ...
          // 将数据存入后台数据库中
          // 将uint8转化为数组
          let uint8Arr = this.leafNodes.map((v) => {
            return JSON.stringify(Array.from(v));
          });
          // 将数组转化为字符串
          let uint8 = JSON.stringify(uint8Arr);
          // 调用接口存数据
