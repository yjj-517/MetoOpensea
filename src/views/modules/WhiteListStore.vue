      ...
      >
        ...
        <div class="dialog_btn">
          <el-input-number
            ...
            label="描述文字"
          ></el-input-number>
          <el-button type="primary" plain @click="onMint">Buy Now</el-button>
        </div>
      </el-dialog>
    </div>
  </div>
  ...
  methods: {
    ...
    async onMint() {
      try {
        ...
        // 从后台获取数据
        let res = await getWhiteList_api({ id: 2 });
        if (res.data.code === 200) {
          let dataAarr = JSON.parse(res.data.datas.uint8);
          // 将数据转化为uint8格式
          this.leafNodes = dataAarr.map((v) => {
            return new Uint8Array(JSON.parse(v));
          });
          // 生成参与 verifyMerkleProof 地址的 Merkle 证明
          let leaf = keccak256(this.account);
          ...
          try {
            ...
            let price1 = this.money1 * this.num;
            const price =
              ethers.utils.parseUnits(price1.toString(), "ether");
            // 估计gasLimit
            let gsl = await contract.TmetoNFTContract.estimateGas.mintWhitelist(
              this.proof,
              this.num,
              {
            ...
            // 调用智能合约验证Merkle证明
            let transaction = await contract.TmetoNFTContract.mintWhitelist(
              this.proof,
              this.num,
